...
* `-subj` - данные субъекта сертификата:
  ...
  * `CN=MyPersonal Root CA` - обычно указывает на владельца сертификата
...
sudo openssl req -new -x509 -nodes -sha512 -days 3650 \
    -subj "/C=RU/CN=MyPersonal Root CA" \
    -keyout ca.key \
    -out ca.crt
```
...
* `-subj` - данные субъекта:
  * `C=RU` - страна Россия
  * `CN=192.168.88.184` - указывает либо на доменное имя,
...
В противном случае образ должен быть успешно загружен на вашу локальную машину.
